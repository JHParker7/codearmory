...
**`NO_PROXY` is injected alongside `HTTP(S)_PROXY`, and has to be.** The IP guard above refuses to *dial* a private address unconditionally — that is the whole point of it, so an allowlisted (or attacker-controlled) name cannot resolve to a cluster service. But some of the endpoints forge itself hands a sandbox *are* cluster-internal ClusterIP URLs: the artifact store the save/restore helper curls, the base-image mirror kaniko is remapped to, the layer-cache repo on it, and any operator-listed insecure/private registry. Routed through the proxy they would be blocked by design. Forge therefore builds the `NO_PROXY` list from those same values (plus `localhost` and `127.0.0.1`) and injects it in both upper- and lower-case forms, so they bypass the proxy and are reached directly over the pod network. Nothing else is exempted — a host forge did not point the sandbox at still goes through the proxy and its IP guard.
