...
Conductor :8080
  ...
  reverse proxy → backend service
    ...
    └── if conductor rejects the request (401):
          ...
          → block IP after 10 failures (1 hour)
        if conductor accepts the request:
          reset source IP suspect counter
```
...
- **Forward signing** — When `CONDUCTOR_FORWARD_KEY` is set, every forwarded request to a `forward_auth=false` service carries `X-Conductor-Token` (HMAC-SHA256 of `conductor:{user_id}:{timestamp}`) and `X-Conductor-Timestamp`, intended to let a backend confirm `X-User-ID` came from Conductor. **Nothing consumes these headers today.** Every registered service sets `forward_auth=true` and re-verifies the forwarded bearer with Gatekeeper directly, so the signing path is written but has no verifier — do not rely on it as a live control.
- **Header stripping** — `X-User-ID`, `X-Conductor-Token`, `X-Conductor-Timestamp`, `X-Forwarded-Host`, `X-Forwarded-Proto`, `X-Real-IP`, `X-Git-Node-Forward` and `Cookie` are stripped from every forwarded request, so a client cannot pre-load any of them. `X-Git-Node-Forward` matters in particular: git-factory treats it as proof that a peer node already authorised the request.
