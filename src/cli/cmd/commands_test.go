...
func TestOrgStateCmd_Get_RunE(t *testing.T) {
	srv, rec := recordingServer(t, http.StatusOK, `{"serial":2}`)
	setupCLI(t, srv)
	silenceStdout(t)

	sub := findSubcmd(t, orgStateCmd, "get")
	if err := sub.RunE(sub, []string{"myorg", "ops", "staging"}); err != nil {
		t.Fatalf("org-state get: %v", err)
	}
	if rec.Method != "GET" || rec.Path != "/myorg/state/ops/staging" {
		t.Errorf("request = %s %s", rec.Method, rec.Path)
	}
}

func TestOrgStateCmd_Push_RunE(t *testing.T) {
	f, err := createTempStateFile(t, `{"serial":3}`)
	if err != nil {
		t.Fatal(err)
	}
	...
	sub := findSubcmd(t, orgStateCmd, "push")
	sub.Flags().Set("file", f) //nolint:errcheck
	if err := sub.RunE(sub, []string{"myorg", "ops", "staging"}); err != nil {
		t.Fatalf("org-state push: %v", err)
	}
	if rec.Method != "POST" || rec.Path != "/myorg/state/ops/staging" {
		t.Errorf("request = %s %s", rec.Method, rec.Path)
	}
}

func TestOrgStateCmd_Push_MissingFile_RunE(t *testing.T) {
	srv, _ := recordingServer(t, http.StatusOK, `{}`)
	setupCLI(t, srv)
	silenceStdout(t)

	sub := findSubcmd(t, orgStateCmd, "push")
	sub.Flags().Set("file", "/no/such/file.tfstate") //nolint:errcheck
	if err := sub.RunE(sub, []string{"myorg", "ops", "staging"}); err == nil {
		t.Fatal("expected error for missing state file, got nil")
	}
}

func TestOrgStateCmd_Delete_RunE(t *testing.T) {
	srv, rec := recordingServer(t, http.StatusOK, `{}`)
	setupCLI(t, srv)
	silenceStdout(t)

	sub := findSubcmd(t, orgStateCmd, "delete")
	if err := sub.RunE(sub, []string{"myorg", "ops", "staging"}); err != nil {
		t.Fatalf("org-state delete: %v", err)
	}
	if rec.Method != "DELETE" || rec.Path != "/myorg/state/ops/staging" {
		t.Errorf("request = %s %s", rec.Method, rec.Path)
	}
}

func TestOrgStateCmd_Lock_RunE(t *testing.T) {
	srv, rec := recordingServer(t, http.StatusOK, `{}`)
	setupCLI(t, srv)
	silenceStdout(t)

	sub := findSubcmd(t, orgStateCmd, "lock")
	sub.Flags().Set("data", `{"ID":"org-lock"}`) //nolint:errcheck
	if err := sub.RunE(sub, []string{"myorg", "ops", "staging"}); err != nil {
		t.Fatalf("org-state lock: %v", err)
	}
	if rec.Method != "LOCK" || rec.Path != "/myorg/state/ops/staging" {
		t.Errorf("request = %s %s", rec.Method, rec.Path)
	}
}

func TestOrgStateCmd_Lock_InvalidData_RunE(t *testing.T) {
	srv, _ := recordingServer(t, http.StatusOK, `{}`)
	setupCLI(t, srv)
	silenceStdout(t)
...
func TestOrgStateCmd_Unlock_RunE(t *testing.T) {
	srv, rec := recordingServer(t, http.StatusOK, `{}`)
	setupCLI(t, srv)
	silenceStdout(t)

	sub := findSubcmd(t, orgStateCmd, "unlock")
	sub.Flags().Set("data", `{"ID":"org-lock"}`) //nolint:errcheck
	if err := sub.RunE(sub, []string{"myorg", "ops", "staging"}); err != nil {
		t.Fatalf("org-state unlock: %v", err)
	}
	if rec.Method != "UNLOCK" || rec.Path != "/myorg/state/ops/staging" {
		t.Errorf("request = %s %s", rec.Method, rec.Path)
	}
}

func TestOrgStateCmd_Unlock_InvalidData_RunE(t *testing.T) {
	srv, _ := recordingServer(t, http.StatusOK, `{}`)
	setupCLI(t, srv)
	silenceStdout(t)
