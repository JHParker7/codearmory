...
// Project scoping (see docs/projects/design.md). A board may be filed into a
...
// — the same forward-the-bearer pattern the read helpers use — because the SDK's
// CheckPermissions writes a 403 on denial and so cannot express an "owner OR project"
// fallback.

const projectService = "tickets"

// authorizeBoard reports whether the caller may act on b: by ownership/tenancy
// (canAccessBoard), or — when b is filed into a real gatekeeper project — by
...
func authorizeBoard(ctx context.Context, bearer, action string, b Board, userID, orgID string) bool {
