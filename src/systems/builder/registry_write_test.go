...
func TestEnsureRegistryService_RegistersWhenAbsent(t *testing.T) {
	...
	def, _ := embeddedServiceDef("forge")
	...
	if post.body["name"] != "forge" {
		t.Errorf("POST name = %v, want forge", post.body["name"])
	}
	if url, _ := post.body["url"].(string); url != "http://codearmory-forge:8083" {
		t.Errorf("POST url = %q, want http://codearmory-forge:8083", url)
	}
...
func TestEnsureRegistryService_NoChurnWhenActive(t *testing.T) {
	...
	stub := &stubRegistry{listBody: `[{"service_id":"svc-1","name":"forge","endpoints":[{"method":"GET","path":"/x"}]}]`}
	useStubRegistry(t, stub)
	def, _ := embeddedServiceDef("forge")
...
func TestEnsureRegistryAccount_Upserts(t *testing.T) {
	stub := &stubRegistry{}
	useStubRegistry(t, stub)
	if err := ensureRegistryAccount(context.Background(), "workflows", "derived-key"); err != nil {
	...
	if got.body["name"] != "workflows" || got.body["key"] != "derived-key" || got.body["role"] != "read" {
