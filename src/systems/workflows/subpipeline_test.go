...
func TestCollectWorkflowPermissions_TriggerGrantsTriggerAndPoll(t *testing.T) {
	...
	perms := collectWorkflowPermissions([]WorkflowStep{{Step: Step{Action: ActionWorkflowsTrigger}}}, nil, nil)
...
func TestCollectWorkflowPermissions_InlineStepContributesPerms(t *testing.T) {
	...
	perms := collectWorkflowPermissions([]WorkflowStep{{Step: Step{Name: "build", Action: "forge/run"}}}, nil, nil)
	var has bool
	for _, p := range perms {
