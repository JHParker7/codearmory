...
// a unix timestamp, and the full request body. Signing the whole body means
// payload/tenant tampering invalidates the token.
...
// enqueueCommand asks the outpost-gateway to deliver a command to the outpost.
// orgID/userID identify the authorizing tenant; the gateway checks them against
// the target outpost's owner so a caller cannot drive another tenant's outpost.
...
// bytes received, with a 30s replay window.
