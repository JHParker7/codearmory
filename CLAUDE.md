...
### Database pattern
All GORM-based services in this repo (`gatekeeper`, `hooks`, `workflows`, `git`, `tickets`, `containers`) — plus the builder-deployed `gitea_integration` — use the same pattern:
...
**Spun off** into their own `codearmory-<svc>` repos (source not in this tree): argo, blueprints, chaos, mcp, notifications, and gitea_integration are deployed + registered by **builder** at runtime. **tickets** and **containers** are **in-repo core** (`src/systems/tickets`, `src/systems/containers`; built by the monorepo CI, Helm-deployed, registered via the registry manifest, no builder def) — they were moved back into the monorepo after being spun off. The core git integration is the in-repo **git** credential broker (`src/systems/git`), not gitea_integration.
